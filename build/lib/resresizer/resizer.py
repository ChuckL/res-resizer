...
class AndroidResResize(BaseResizer):
    ...
    def process_file(self, input_directory, file_name):
        ...
        directory_scale = "xxxhdpi"
        if "xxxhdpi" in input_directory:
            directory_scale = "xxxhdpi"
        elif "xxhdpi" in input_directory:
            directory_scale = "xxhdpi"
        elif "xhdpi" in input_directory:
            directory_scale = "xhdpi"
        elif "hdpi" in input_directory:
            directory_scale = "hdpi"
        else:
            directory_scale = "mdpi"

        directory_scale_value = self.SCALES.items[directory_scale]

        file_path = os.path.join(input_directory, file_name)
        base_name, file_extension = os.path.splitext(file_path)
        if self.can_process_file(file_extension):
            for scale_name, scale_value in self.SCALES.items():
                if scale_value >= directory_scale_value:
                    continue;

                new_image = self.scale_image(file_path, scale_value / directory_scale_value)
